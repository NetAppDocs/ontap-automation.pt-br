...
Segurança:: Diversas melhorias de segurança foram introduzidas com a ONTAP 9.18.1 API REST. Você pode usar os novos endpoints de rede do cluster para exibir e atualizar a configuração de segurança da rede do cluster, incluindo o gerenciamento de certificados. Além disso, você pode administrar a configuração de segurança da rede HA para o tráfego NVLog. A Proteção Autônoma contra Ransomware (ARP) foi aprimorada para permitir que você exiba e atualize o recurso de capacitação.
