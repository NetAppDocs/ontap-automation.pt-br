...
A versão 9.18.1 do ONTAP continua a expandir as capacidades da API REST do ONTAP com onze novas chamadas de API. As melhorias focam-se principalmente na segurança, mas também incluem uma otimização relacionada à administração de armazenamento.

Segurança:: Existem duas grandes melhorias de segurança introduzidas com a API REST do ONTAP 9.18.1. Você pode usar os novos endpoints da API de rede do cluster para exibir e atualizar a configuração de segurança da rede do cluster. O sistema inclui suporte robusto para gerenciamento de certificados na rede do cluster. Além disso, você pode exibir e atualizar a configuração de segurança da rede HA para o tráfego NVLog.
