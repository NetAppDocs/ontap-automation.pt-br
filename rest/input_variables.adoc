...
Ao fazer uma chamada de API usando GET para acessar uma coleção de objetos do mesmo tipo, o ONTAP tenta retornar o máximo de objetos possível com base em duas restrições (número máximo de objetos, tempo usado para recuperar os objetos). Você pode controlar cada uma delas usando parâmetros de consulta adicionais na solicitação. A primeira restrição atingida para uma solicitação GET específica encerra a solicitação e, portanto, limita o número de registros retornados. Se uma solicitação terminar antes de iterar sobre todos os objetos, a resposta contém o link necessário para recuperar o próximo lote de registros.


TIP: Se zero registros forem retornados em uma chamada à API, sem nenhum próximo link, isso pode indicar que ocorreu um tempo limite antes que qualquer registro pudesse ser obtido. Nesse caso, você pode tentar novamente a solicitação usando um valor  `return_timeout`maior ou um valor  `max_records`menor (ou ambos).
