...
.Capacitação anti-ransomware
Você pode controlar a operação do recurso de capacitação da Proteção Autônoma contra Ransomware (ARP). Isso inclui recuperar e modificar as configurações. Esse tipo de recurso foi introduzido com o ONTAP 9.18.1.
...
.Segurança de rede em cluster
Você pode recuperar e atualizar a configuração de segurança da rede do cluster, incluindo os certificados. Este tipo de recurso foi introduzido com o ONTAP 9.18.
...
.Segurança de rede HA
Você pode recuperar e atualizar a configuração de segurança da rede HA. Este tipo de recurso foi introduzido com o ONTAP 9.18.
...
.Diretivas IPsec
Você pode usar esse conjunto de chamadas de API para gerenciar as políticas em vigor em uma implementação IPSec. Esse tipo de recurso é novo no ONTAP 9.8. Elas foram atualizadas na versão 9.19 para recuperar as políticas usadas na rede do cluster.

.Associações de segurança IPsec
Você pode usar esse conjunto de chamadas de API para gerenciar as associações de segurança em vigor para uma implementação IPSec. Esse tipo de recurso é novo no ONTAP 9.8. Elas foram atualizadas na versão 9.19 para recuperar as associações de segurança usadas na rede do cluster.
...
.Autenticação da Web
A autenticação na web (WebAuthn) é um padrão web para autenticar usuários com segurança, baseado em criptografia de chave pública. Com ONTAP, ela oferece suporte à administração de MFAs resistentes a phishing por meio do System Manager e da API REST do ONTAP. Esse recurso foi adicionado no ONTAP 9.16 e atualizado no ONTAP 9.19.

.Quem sou eu
Um endpoint está disponível para exibir os detalhes do usuário autenticado, incluindo nome de usuário, função e permissões. Esse tipo de recurso foi introduzido no ONTAP 9.19.
