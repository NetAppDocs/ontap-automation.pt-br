...
.Autenticação da Web
A autenticação web (WebAuthn) é um padrão web para autenticar usuários com segurança, baseado em criptografia de chave pública. Com o ONTAP, ela oferece suporte à administração de MFAs resistentes a phishing por meio do System Manager e da API REST do ONTAP. Esse recurso foi adicionado no ONTAP 9.16 e atualizado no ONTAP 9.19.1.

.Quem sou eu
Um endpoint está disponível para exibir os detalhes do usuário autenticado, incluindo nome de usuário, função e permissões. Esse tipo de recurso foi introduzido no ONTAP 9.19.1.
