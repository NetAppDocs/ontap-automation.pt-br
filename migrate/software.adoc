...
O NetApp.ONTAP PowerShell Toolkit aprimora seu ambiente local do PowerShell com um módulo que inclui mais de 2.400 cmdlets. Ele permite que você desenvolva rapidamente código para seu host Windows para automatizar as implantações do ONTAP. Para obter mais informações, consulte link:../pstk/learn-about-pstk.html["Saiba mais sobre o NetApp ONTAP PowerShell Toolkit"].
